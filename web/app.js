...
function renderGisMonthly(d){
  ...
  var eh=d&&d.external&&d.external.directEcommerceHistory;
  var ecommerceByYm={}; (eh||[]).forEach(function(x){ if(x&&x.ym) ecommerceByYm[x.ym]=x.utm2gis||null; });
  ...
  var MM=['','Янв','Фев','Мар','Апр','Май','Июн','Июл','Авг','Сен','Окт','Ноя','Дек'];
  var imp=0,pv=0,sc=0,rt=0,cl=0,so=0,me=0,pr=0,ep=0,er=0;
  var selYM=mktSelectedPeriod();
  var hlRow=function(ym){ return ym===selYM?' style="background:var(--accent-soft);box-shadow:inset 3px 0 0 var(--accent)"':''; };
  var nv=function(v){ return v==null?'—':mNum(v); };
  var rows=ms.map(function(m){ var e=ecommerceByYm[m.ym]||{}; var purch=e.purchases, rev=e.purchaseRevenue; imp+=m.impressions||0; pv+=m.pageVisits||0; sc+=m.siteClicks||0; rt+=m.routes||0; cl+=m.calls||0; so+=m.socialClicks||0; me+=m.messengerClicks||0; pr+=m.priceViews||0; ep+=purch||0; er+=rev||0;
    var p=m.ym.split('-'); var lbl=p[0].slice(2)+'-'+MM[Number(p[1])]+(m.partial?' <span style="font-size:10px;color:var(--muted)">(идёт)</span>':'');
    return '<tr'+hlRow(m.ym)+'><td>'+lbl+'</td><td class="num">'+mNum(m.impressions)+'</td><td class="num">'+(m.positionAvg==null?'—':mNum1(m.positionAvg))+'</td><td class="num">'+nv(m.pageVisits)+'</td><td class="num">'+nv(m.siteClicks)+'</td><td class="num">'+nv(m.routes)+'</td><td class="num">'+nv(m.calls)+'</td><td class="num">'+nv(m.socialClicks)+'</td><td class="num">'+nv(m.messengerClicks)+'</td><td class="num">'+nv(m.priceViews)+'</td><td class="num">'+(purch==null?'—':mNum(purch))+'</td><td class="num">'+(rev==null?'—':mNum(rev)+' ₽')+'</td></tr>';
  }).reverse().join('');
  ...
  el.innerHTML='<div class="mkt-chart-t">2ГИС помесячно — показы, позиция, переходы <span class="mkt-scope dyn">live · кабинет · 13 мес</span></div>'+
    '<div class="table-wrap"><table style="font-size:12px"><thead><tr><th>Месяц</th><th class="num" title="Показы карточки в поиске 2ГИС за календарный месяц">Показы</th><th class="num" title="Средняя позиция в выдаче за месяц">Позиция ср.</th><th class="num" title="Переходы на страницу компании в 2ГИС">Переходы на стр.</th><th class="num" title="Клики «перейти на сайт» с карточки">На сайт</th><th class="num" title="Построения маршрута до точки — намерение прийти">Маршруты</th><th class="num" title="Звонки и просмотры телефона">Звонки</th><th class="num" title="Клики по ссылкам соцсетей на карточке">Соцсети</th><th class="num" title="Клики по ссылкам мессенджеров">Мессендж.</th><th class="num" title="Просмотры блока цен на карточке">Цены</th><th class="num" title="Покупки сайта с utm_source=2gis">Покупки</th><th class="num" title="Выручка сайта с utm_source=2gis">Выручка ₽</th></tr></thead><tbody>'+rows+
    '<tr class="mkt-total"><td>Итого</td><td class="num">'+mNum(imp)+'</td><td class="num"></td><td class="num">'+mNum(pv)+'</td><td class="num">'+mNum(sc)+'</td><td class="num">'+mNum(rt)+'</td><td class="num">'+mNum(cl)+'</td><td class="num"></td><td class="num"></td><td class="num">'+mNum(pr)+'</td><td class="num">'+mNum(ep)+'</td><td class="num">'+mNum(er)+' ₽</td></tr>'+
    '</tbody></table></div>'+
    '<div style="font-size:11px;color:var(--muted);margin-top:6px">Календарные месяцы из кабинета 2ГИС («Присутствие в выдаче» + «Страница компании», пресет «Год», обновляется ежедневно). Покупки и выручка — ecommerce Метрики по <b>utm_source=2gis</b> (только сайт). Офлайн-касса в эти колонки не входит. Обновлено: '+st+'.</div>';
}
...
function renderMarketingBudget(b){
  ...
  var t=b.totals||{}, plan=b.plan||{}, fact=b.fact||{}, paid=b.paid||{};
  ...
  if(cats.length){
    var planItems=plan.items||[], orders=fact.orders||[], paidArticles=paid.articles||[];
    ...
    var detailsFor=function(c){
      var pRows=planItems.filter(function(x){return x.category===c.key;});
      var oRows=orders.filter(function(x){return x.category===c.key;});
      var aRows=paidArticles.filter(function(x){return x.category===c.key;});
      var out='<div class="mkt-budget-detail-grid">';
      if(pRows.length){
        out+='<div><div class="mkt-budget-detail-title">Плановые статьи · '+mNum(pRows.length)+'</div><div class="table-wrap"><table style="font-size:11px"><thead><tr><th>Статья</th><th>Комментарий</th><th class="num">План, ₽</th></tr></thead><tbody>'+pRows.map(function(i){return '<tr><td><b>'+escapeHtml(i.name)+'</b><div style="color:var(--muted)">'+escapeHtml(i.section||'')+'</div></td><td>'+escapeHtml(i.comment||'—')+'</td><td class="num">'+mNum(i.amount)+'</td></tr>';}).join('')+'</tbody></table></div></div>';
      }
      if(oRows.length){
        out+='<div><div class="mkt-budget-detail-title">Заявки финансового планирования · '+mNum(oRows.length)+'</div><div class="table-wrap"><table style="font-size:11px"><thead><tr><th>Дата / №</th><th>Фонд и предмет</th><th>Статус</th><th class="num">Запрошено</th><th class="num">Утверждено</th></tr></thead><tbody>'+oRows.map(function(o){var kind=o.statusKind||'pending';return '<tr><td style="white-space:nowrap">'+escapeHtml(String(o.date||'').slice(0,10))+'<div style="color:var(--muted)">'+escapeHtml(o.number||'')+'</div></td><td><b>'+escapeHtml(o.fund||'—')+'</b><div style="color:var(--muted)">'+escapeHtml(o.subject||'—')+'</div></td><td style="color:'+(statusColor[kind]||'var(--muted)')+'"><b>'+(statusLabel[kind]||escapeHtml(o.status||'—'))+'</b></td><td class="num">'+mNum(o.requested||0)+'</td><td class="num">'+mNum(o.approved||0)+'</td></tr>';}).join('')+'</tbody></table></div></div>';
      }
      if(aRows.length){
        var tx=[];
    ...
    html+='<div class="section-hint" style="margin-top:12px">Нажмите на категорию, чтобы раскрыть все плановые статьи, заявки ФП и фактические движения денег.</div>'+
      ...
      cats.map(function(c){
        ...
        var id='mkt-budget-detail-'+c.key;
        return '<tr class="mkt-budget-cat-row" data-target="'+id+'" tabindex="0" role="button" aria-expanded="false" aria-controls="'+id+'"><td><span class="mkt-budget-arrow">▸</span><b>'+escapeHtml(c.name)+'</b></td><td class="num">'+mNum(c.plan)+'</td><td class="num">'+mNum(c.approved)+'</td><td class="num"><b>'+mNum(c.paid)+'</b></td><td class="num">'+mNum(c.pending)+'</td><td class="num" style="'+remColor+'">'+mNum(c.remainingPaid)+'</td><td class="num">'+pct(c.paidPct)+'</td></tr><tr id="'+id+'" class="mkt-budget-cat-detail" hidden><td colspan="7">'+detailsFor(c)+'</td></tr>';
      }).join('')+
      '<tr class="mkt-total"><td><b>Итого</b></td><td class="num"><b>'+mNum(t.plan)+'</b></td><td class="num"><b>'+mNum(t.approved)+'</b></td><td class="num"><b>'+mNum(t.paid)+'</b></td><td class="num"><b>'+mNum(t.pending)+'</b></td><td class="num" style="color:'+remainingColor+'"><b>'+mNum(t.remainingPaid)+'</b></td><td class="num"><b>'+pct(t.paidPct)+'</b></td></tr>'+
      '</tbody></table></div>';
  }
  ...
  el.querySelectorAll('.mkt-budget-cat-row').forEach(function(row){
    var toggle=function(){
      var detail=document.getElementById(row.dataset.target); if(!detail) return;
...
function mktExport(){
  ...
  // Маркетинговый бюджет: план Google Sheets + ФП + фактические оплаты 1С.
  if(_mktBudget && _mktBudget.totals){
    var bt=_mktBudget.totals;
    push(['Бюджет маркетинга — план / факт']);
    push(['План руб','Утверждено на ФП руб','Фактически оплачено руб','Ожидает ФП руб','Остаток после оплат руб','Оплачено %']);
...
var COMPETITORS = [
  ...
  { name:'Стефания', site:'stefanycake.ru', points:'~31–39 (Иркутск, Ангарск, Шелехов, Усолье, Черемхово)',
    ...
    loyalty:'«Кофейная карта», −10 % на пироги от 5 кг; есть регистрация чека по QR (бонусы/розыгрыш) — но прозрачной многоуровневой накопительной программы как у «Марии» публично не видно',
  ...
  { name:'Cake Home', site:'cakehome.ru', points:'~20–24 (Иркутск, Ангарск, Шелехов, Хомутово)',
    social:'IG @my_cakehome 38 399; VK', followers:'38 399 (IG)', rating:'2ГИС 4,6 (~90)',
    ...
    promos:'н/д', strong:'единственный с конструктором торта, семейный крафт-образ, широкая сеть, сильный IG (38к, виральные Reels)',
    weak:'участившиеся жалобы на вкус/качество тортов (сухой бисквит, «испорченный вкус»), рост цен' },
...
var COMP_INSIGHTS = [
  '<b>Лояльность — главное преимущество «Марии»:</b> единственный игрок с прозрачным многоуровневым кэшбэком (5/7/10 %, оплата до 30 % бонусами) + клуб на 100к+. У конкурентов такой программы публично нет (у Стефании — лишь регистрация чеков по QR и «кофейная карта»; у Этики/ЯХОНТа — не найдено). Это конкурентный ров, его стоит активно продвигать.',
...
// Отзывы — обновлено по свежим отзывам 2ГИС (июнь 2026); рейтинг = live-агрегат по сети.
var REVIEWS = {
  'Мария': { rating:'2ГИС 4,5 (6 075)', pros:'вкус десертов (Банан-солёная карамель, медовик), быстрое обслуживание, бесплатная доставка, игровая зона для детей', cons:'жалобы на качество/свежесть отдельных позиций (недомогание от пирожного со сливочным кремом), реже — обслуживание' },
  'Стефания': { rating:'2ГИС 4,8 (20 893) — топ рынка', pros:'цена-качество, свежесть, большой выбор, вежливый персонал, можно есть в зале или с собой', cons:'не всегда отдают чек с QR для регистрации бонусов; нарекания на молодой персонал (наушники/телефон)' },
  'Этика': { rating:'2ГИС 4,7 (4 793)', pros:'вкус, атмосфера и вид на Ангару, вежливые официанты, завтраки с 8:00, кофе', cons:'платный пакет «на вынос» без предупреждения; ранее — высокие цены и порции' },
  'Cake Home': { rating:'2ГИС 4,4 (6 457)', pros:'торты «Полёт»/«Анна Павлова», профессиональные бариста, уют', cons:'участились жалобы на ВКУС/качество (сухой бисквит, «испорченный вкус»), дорого (478 ₽/кусок)' },
  'ЯХОНТ': { rating:'2ГИС 4,6 (1 321)', pros:'огромный выбор тортов, натуральность, свежесть, вежливые продавцы, эклеры', cons:'нарекания на качество отдельных тортов (сухой «Красный бархат»); исторически срывы сроков' }
};
function renderCompetitors(d){
  ...
  var srcLine = gcAt ? '<div style="font-size:11px;color:var(--muted);margin:6px 0">Точки · рейтинг · число отзывов — <b>live из 2ГИС</b> (обновлено '+String(gcAt).slice(0,10)+'). Подписчики Telegram — live, Instagram — снимок 24.06. Лояльность · акции · сильные/слабые · «хвалят/ругают» — анализ открытых источников и отзывов 2ГИС, <b>июнь 2026</b>. Выручка и охваты соцсетей — оценка (публично не раскрываются).</div>' : '';
...
var SOCIAL = [
  ...
  { name:'Cake Home', key:'cakehome', tg:'н/д', tgReach:'~700–1 100', ig:'38 399', igReels:'~21,7 тыс', vk:'н/д' },
...
function renderSocial(d){
  ...
  el.innerHTML='<div class="table-wrap"><table><thead><tr>'+th+'</tr></thead><tbody>'+trs+'</tbody></table></div>'+srcLine+
    '<div class="mkt-comp-ins" style="margin-top:12px"><b>Вывод по соцсетям:</b> по видео «Мария» отстаёт. <b>Telegram</b>: подписчиков ×'+tgRatio+' меньше Стефании (live) — самый недоиспользованный канал. <b>IG Reels</b> (оценка): у «Марии» ~2,3 тыс просм./рилс — заметно ниже Cake Home (~21,7к, самые виральные) и Стефании (~8,1к); выше Этики (~1,4к) и ЯХОНТа (~0,2к). <b>Instagram</b>: 31 699 подписчиков — 3-е место, после Стефании (44 865) и Cake Home (38 399), выше Этики (22 353) и ЯХОНТа (4 072). VK-охваты приватны.</div>';
}
var TOP_PRODUCTS = [
...
var PRICES = [
  ...
  ['Просто торты (готовые, в наличии)*','<span title="1С, чеки 25.05–24.06.2026 (30 дн): 42 позиции, от 1247 ₽, ходовые (топ-10 по штукам) 1363–1932 ₽, средняя по позициям 2274 ₽">от 1 247 ₽/шт · ходовые 1 363–1 932 ₽</span>','<span title="stefanycake.ru «Торты на каждый день», 20+ позиций, 05.06.2026; большинство 765–1 195 ₽/кг">525–1 350 ₽/кг</span>','<span title="etikacakes.ru «Стандартные торты», 8 позиций по 1–1,6 кг, 05.06.2026 (≈1 900–2 280 ₽/кг)">2 280–3 280 ₽/шт</span>','<span title="cakehome.ru/catalog/cakes, 05.06.2026; целиком 848–4 485 ₽/шт">1 690–2 990 ₽/кг</span>','<span title="Официальная карточка 2ГИС (Обручева 14), 21 позиция, 05.06.2026; большинство 900–1 305 ₽">640–1 700 ₽/шт</span>'],
  ['Бенто-торт','от 690 ₽','1000 ₽','от 1590 ₽','н/д <span style="font-size:10px;color:var(--muted)">нет онлайн</span>','от 495 ₽ <span style="font-size:10px;color:var(--muted)">снимок 05.06</span>'],
  ['Кусочек / пирожное','89–423 ₽','89–199 ₽','210–460 ₽','н/д','н/д'],
  ['Макаронс, ₽/шт','н/д','109 ₽','135 ₽','120 ₽ <span style="font-size:10px;color:var(--muted)">снимок 05.06</span>','131 ₽ <span style="font-size:10px;color:var(--muted)">снимок 05.06</span>'],
  ['Капучино','170–420 ₽','н/д','н/д','н/д','н/д']
];
...
function mktLoadYoY(){
  ...
  fetchJson('/api/marketing/channels?period='+period).then(function(d){
    ...
    // Бюджет: план Google Sheets + последнее решение ФП + фактические оплаты 1С.
