  ...
  try {
    ...
    if (pathname === '/api/production/plan' && req.method === 'GET') {
      try {
        ...
        if (parsedUrl.searchParams.get('demand') === 'hub') opts.demand = 'hub';
    ...
    if (pathname === '/api/production/convergence' && req.method === 'GET') {
      try {
        ...
        const demand = parsedUrl.searchParams.get('demand') === 'hub' ? 'hub' : 'central';
        sendJson(res, 200, await productionPlan.getConvergence(date, demand));
      } catch (e) { sendJson(res, 500, { error: e.message }); }
      return;
    }

    // Дневной план выпуска в .xlsx (скачивание / для cron-доставки Маше).
    if (pathname === '/api/production/plan.xlsx' && req.method === 'GET') {
      try {
        let date = parsedUrl.searchParams.get('date');
        if (!date || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
          const t = new Date(Date.now() + 86400000);
