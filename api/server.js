...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/admin/returns-probe' && req.method === 'GET') {
      ...
      const lineQ = 'ВЫБРАТЬ Т.Ссылка.ВидОперации КАК Вид,'
        + ' СУММА(ЕСТЬNULL(Т.Сумма,0)) КАК A_сумма,'
        + ' СУММА(ЕСТЬNULL(Т.Сумма,0) - ЕСТЬNULL(Т.ОплатаБонусами,0) - ЕСТЬNULL(Т.ОплатаПодарочнымиСертификатами,0)) КАК B_безБонСерт,'
        + ' СУММА(ВЫБОР КОГДА Т.РегистрацияПродажи ТОГДА ЕСТЬNULL(Т.Сумма,0) ИНАЧЕ 0 КОНЕЦ) КАК C_толькоРегПродажи,'
        + ' СУММА(ВЫБОР КОГДА Т.Подарок ТОГДА 0 ИНАЧЕ ЕСТЬNULL(Т.Сумма,0) КОНЕЦ) КАК D_безПодарков,'
        + ' СУММА(ЕСТЬNULL(Т.ОплатаБонусами,0)) КАК Бонусы,'
        + ' СУММА(ЕСТЬNULL(Т.ОплатаПодарочнымиСертификатами,0)) КАК Серт,'
      ...
      const hdrQ = 'ВЫБРАТЬ Чек.ВидОперации КАК Вид, СУММА(ЕСТЬNULL(Чек.СуммаДокумента,0)) КАК СуммаДок,'
