...
// Запрос списка отправок месяца — те же поля что в sms-attribution (без критерия).
...
// Отдельный запрос за критериями: без агрегации (МАКСИМУМ над строкой
// неограниченной длины ломал 1С). Тянем все маркетинговые SMS месяца с
// их полями. Потом мапим к кампаниям по нормализованному тексту.
function criteriaQuery(period) {
  const b = monthBounds(period);
  return 'ВЫБРАТЬ С.Дата КАК Дата, С.Тема КАК Тема,'
    + ' ВЫРАЗИТЬ(С.ТекстПисьма КАК СТРОКА(300)) КАК Текст,'
    + ' С.СодержаниеТемыSMS КАК Критерий,'
    + ' С.Ответственный.Наименование КАК Ответственный'
    + ' ИЗ Документ.SMSСообщение КАК С'
    + ` ГДЕ С.Дата >= ДАТАВРЕМЯ(${b.y},${b.m},1) И С.Дата < ДАТАВРЕМЯ(${b.ny},${b.nm},1)`
    + ' УПОРЯДОЧИТЬ ПО С.Дата';
}
...
function audienceQuery({ dtsLits, dtLitNow, d90, d365 }) {
  // ВАЖНО: получателей дедупим подзапросом РАЗЛИЧНЫЕ ДО джойна с чеками. Иначе при
  // ресендах (одна карта в нескольких отправках) П-строки множатся → СУММА(СуммаДокумента)
  // ×N → карта ложно попадает в VIP. РАЗЛИЧНЫЕ + ЛЕВОЕ СОЕДИНЕНИЕ сохраняет «холодных»
  // (без покупок: МАКСИМУМ(Дата)=NULL).
  return 'ВЫБРАТЬ Сегмент, КОЛИЧЕСТВО(*) КАК Карт ИЗ ('
    ...
    + ` ИНАЧЕ "холодный"`
    + ' КОНЕЦ КАК Сегмент'
    + ' ИЗ (ВЫБРАТЬ РАЗЛИЧНЫЕ П2.Получатель КАК К ИЗ Документ.SMSСообщение.Получатели КАК П2'
    + ` ГДЕ П2.Ссылка.Дата В (${dtsLits.join(', ')})) КАК П`
    + ' ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЧекККМ КАК Чек'
    + ' ПО Чек.ДисконтнаяКарта = П.К'
    + ` И Чек.Дата >= ${d365} И Чек.Дата < ${dtLitNow} И Чек.Проведен`
    + ' СГРУППИРОВАТЬ ПО П.К'
    + ') КАК Т'
...
async function compute(period) {
  ...
  // Параллельно — отдельный запрос за критериями/ответственными (без агрегации).
  let critRows = [];
  try {
    const cr = await upp.callQuery(criteriaQuery(period));
    critRows = (cr && cr.rows) || [];
  } catch (_) { /* падение не блокирует основной поток */ }
  ...
  // Мапим критерии: ключ — нормализованный текст, берём ПЕРВУЮ непустую запись.
  for (const r of critRows) {
    if (!MARKETING_RE.test(r.Тема || '')) continue;
    const key = normText(r.Текст);
