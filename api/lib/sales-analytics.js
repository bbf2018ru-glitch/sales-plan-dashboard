...
function filterDbByDateRange(db, from, to) {
  ...
  return {
    ...
    sales: db.sales.filter(r => {
      ...
      // Сюда попадаем только когда задан from/to (см. ранний return выше при их отсутствии).
      // Строку без валидной даты НЕЛЬЗЯ отнести к выбранному окну → отсекаем. Раньше
      // оставляли (return true), и недатированные строки попадали в КАЖДЫЙ под-диапазон
      // месяца → завышение сумм / двойной счёт между диапазонами. Во вью «весь месяц»
      // (from/to не заданы) такие строки сохраняются — там фильтр не доходит до сюда.
