...
const HDR = ['Наименование', 'ГП', 'Запас', 'Контейнер', 'Вычерки', 'Довозы', 'Сайт завтра',
  'Отгрузка завтра (прогноз)', 'Остаток-прогноз', 'Послезавтра (прогноз)', 'Сайт послезавтра', 'Выходные', 'ИТОГО', 'Задание'];

function v(x) { return (x == null || x === 0) ? null : x; }

function buildXlsx(plan) {
  const XLSX = require('xlsx'); // ленивый require — отсутствие пакета не валит весь сервис
  const aoa = [];
  aoa.push(['План выпуска кондитерки · ' + plan.date + ' (' + plan.weekday + ') · послезавтра ' + plan.nextWeekday
    + '   |   данные из 1С: остаток — текущий; «Отгрузка» — ПРОГНОЗ (среднее за 3 нед); Вычерки/Довозы/Выходные — заполнить вручную']);
  aoa.push(HDR);
