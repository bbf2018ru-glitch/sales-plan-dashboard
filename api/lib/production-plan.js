...
async function qStock() {
  ...
  // Флаг «готовая продукция» (НоменклатурнаяГруппа.ЭтоПродукция) — для цехов берём в контейнер
  // ТОЛЬКО готовую продукцию (не сырьё/п-ф/компоненты вроде «Медалька»/«мусс п/ф»).
  const q = 'ВЫБРАТЬ Ост.Склад.Код КАК Склад, Ост.Номенклатура.Код КАК Код, Ост.Номенклатура.Наименование КАК Имя,'
    + ' Ост.Номенклатура.НоменклатурнаяГруппа.ЭтоПродукция КАК Прод,'
    + ' СУММА(Ост.КоличествоОстаток) КАК Кол'
    + ` ИЗ РегистрНакопления.ТоварыНаСкладах.Остатки(, Склад.Код В (${wh})) КАК Ост`
    + ' СГРУППИРОВАТЬ ПО Ост.Склад.Код, Ост.Номенклатура.Код, Ост.Номенклатура.Наименование, Ост.Номенклатура.НоменклатурнаяГруппа.ЭтоПродукция'
...
// L/P (сайт-заказы) = пресс-сетка САЙТ из регистра ф_ЗаказыДляПрессСетки (то, что читает
// Отчёт.ф_ПрессСетка → лист «Завтра/Послезавтра сайт»). Рейсы «сайт» (доставка клиенту) и
// «сайт для склада» (самовывоз в точке). Период = дата отгрузки. Прежний фильтр по контрагенту
// «(сайт)» был неверен: ловил лишнее (сертификаты/пакеты) и пропускал именные заказы/блюда.
// НЕ путать с магазинной пресс-сеткой (Рейс «Наши торговые точки» = M/O, тянется отдельно).
async function qSite(day) {
  const q = 'ВЫБРАТЬ Р.Номенклатура.Код КАК Код, Р.Номенклатура.Наименование КАК Имя, СУММА(Р.Количество) КАК Кол'
    + ' ИЗ РегистрНакопления.ф_ЗаказыДляПрессСетки КАК Р'
    + ` ГДЕ Р.Период >= ${dtLit(day)} И Р.Период < ${dtLit(addDays(day, 1))}`
    + ' И Р.Рейс.Наименование В ("сайт", "сайт для склада")'
    + ' СГРУППИРОВАТЬ ПО Р.Номенклатура.Код, Р.Номенклатура.Наименование'
    + ' ИМЕЮЩИЕ СУММА(Р.Количество) > 0';
...
async function compute(dateStr, opts) {
  ...
  // остаток по коду. H (Контейнер) = остатки на производстве (склады-цеха), ТОЛЬКО готовая
  // продукция (ЭтоПродукция=Да) — сырьё/п-ф/компоненты в контейнер не идут.
  const kontSet = new Set(kontCodes);
  const isProd = (r) => { const v = cc(r.Прод); return v === 'Да' || v === 'true' || v === 'Истина'; };
  const E = {}, G = {}, H = {}, names = {};
  for (const r of stockRows) {
    ...
    else if (kontSet.has(w) && isProd(r)) H[code] = (H[code] || 0) + q;
  }
...
// ВАЖНО: сортировка — по ДАТЕ убыв (самый СВЕЖИЙ полный день), а не по числу строк.
// Раньше сортировали `Строк УБЫВ` → дефолт залипал на дне с максимумом строк (напр.
// 28.05), даже когда есть свежее полное планирование (09.06) — кнопка обещает «за
// прошлый день», а показывала 2-недельную давность. HAVING ≥50 уже отсекает частичные
// доки, поэтому среди полных дней правильный выбор — последний по дате.
async function lastFullArmDate() {
  const q = 'ВЫБРАТЬ А.ДатаНачала КАК Д, КОЛИЧЕСТВО(*) КАК Строк'
    ...
    + ' СГРУППИРОВАТЬ ПО А.ДатаНачала ИМЕЮЩИЕ КОЛИЧЕСТВО(*) >= 50 УПОРЯДОЧИТЬ ПО А.ДатаНачала УБЫВ';
  const rows = (await upp.callQuery(q, { timeoutMs: 60000 })).rows || [];
