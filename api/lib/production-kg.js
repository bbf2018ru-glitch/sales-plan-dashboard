...
// — он пуст). Кг = Σ(Количество × Справочник.Номенклатура.ф_ВесШтукивКг).
...
async function compute(period) {
  ...
  return {
    ...
    note: 'Выпуск из Документ.ОтчетПроизводстваЗаСмену. Кг = Σ(Количество × вес штуки, кг). Позиции без заданного веса в кг не входят.',
...
function getProductionKg(period) {
  ...
  return cache.wrap('prodkg:' + p, () => compute(p));
}
