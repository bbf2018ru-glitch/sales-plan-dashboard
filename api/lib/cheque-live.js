...
async function build(period) {
  ...
  const hq = await callQuery(
    `ВЫБРАТЬ
       ...
       ВЫРАЗИТЬ(Ч.Склад.ВидСклада.Наименование КАК СТРОКА(60)) КАК Формат,
       КОЛИЧЕСТВО(Ч.Ссылка) КАК Чеков,
     ...
     ГДЕ ${WH}
     СГРУППИРОВАТЬ ПО Ч.Склад.Код, ВЫРАЗИТЬ(Ч.Склад.Наименование КАК СТРОКА(80)), ВЫРАЗИТЬ(Ч.Склад.ВидСклада.Наименование КАК СТРОКА(60))
