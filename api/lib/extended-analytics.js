...
async function buildPromoByAction(fromYM, toYM) {
  ...
  // не ПустаяСсылка мало — есть реальный элемент Акции с пустым Наименованием (база-«нет акции»,
  // ~95к строк/мес); фильтруем по непустому имени, иначе KPI завышаются почти до всех чеков.
  const NOTEMPTY = `${ACT} <> ""`;
